...
const mapSourceWithTarget = ({locales, translations, sourceKey, targetKey}) => locales.map(locale => {
    const result = translations.find(t => t[sourceKey].toLowerCase() == locale.source.toLowerCase())
...
Object.keys(MESSAGES).forEach(targetKey => {
    ...
    MESSAGES[targetKey].forEach(fileName => writeLocale({
        fileName,
        translations,
        targetKey
    }))
